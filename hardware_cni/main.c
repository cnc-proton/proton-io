...
// =========================================================
// ⚙️ НАСТРОЙКА ТИПА МОДУЛЯ
// Выберите цифру в зависимости от вашей платы:
//   1 = 8 входов (P0) и 16 выходов (P1, P2)  [Стандартная]
//   2 = 8 входов (P0) и 8 выходов (P1)       [Младшая версия]
//   3 = 16 входов (P1, P2) и 8 выходов (P0)  [Модуль наоборот]
// =========================================================
...
// =========================================================
// ГЛОБАЛЬНЫЕ НАСТРОЙКИ (Изменяются по Modbus)
// =========================================================
volatile unsigned char cfg_wd_ticks = 10; // Таймаут Watchdog (10 тиков * ~40мс = 400мс)
volatile unsigned char cfg_tx_delay = 10; // Задержка удержания TX для стоп-бита (циклы nop)
...
void timer0_isr(void) __interrupt(1) {
    ...
    led_timer++;
    if (comm_watchdog < cfg_wd_ticks) {
        if (led_timer >= 5) { // 5 Гц (связь есть)
            base_led_state = !base_led_state; WD_LED = base_led_state; led_timer = 0;
        }
    } else {
        if (led_timer >= 50) { // 0.5 Гц (ошибка связи)
            base_led_state = !base_led_state; WD_LED = base_led_state; led_timer = 0;
        }
    }
}
...
// =========================================================
// ЧТЕНИЕ АДРЕСА C DIP-ПЕРЕКЛЮЧАТЕЛЕЙ
// =========================================================
...
// =========================================================
// ОТПРАВКА ОТВЕТА (ОПТИМИЗИРОВАНО ДЛЯ 750k BAUD)
// =========================================================
void Send_Response(unsigned char len) {
    unsigned char i; unsigned int delay;
    
    DIR_PIN = 0; // Переводим MAX485 в режим передачи (TX)
    ...
    // Ждем вылета стоп-бита (используем настраиваемую переменную!)
    for(delay = 0; delay < cfg_tx_delay; delay++) __asm nop __endasm; 
    
    RI = 0;      // СНАЧАЛА чистим возможный мусор в буфере
    DIR_PIN = 1; // ЗАТЕМ возвращаем MAX485 в режим приема (RX)
}
            ...
            if (rx_index == 8) {
                ...
                if (calc_crc == rx_crc) {
                    ...
                    if (rx_buffer[1] == 0x03) { 
                        ...
                        else if (start_reg == 2) { // НАСТРОЙКА WATCHDOG
                            if (rx_buffer[5] > 0) cfg_wd_ticks = rx_buffer[5];
                        }
                        else if (start_reg == 3) { // НАСТРОЙКА ПАУЗЫ TX
